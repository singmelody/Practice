...
RagDoll::RagDoll(WCHAR* fileName, D3DXMATRIX& world) 
	: SkinnedMesh()
{
	Load(fileName);
	SetPose(world);
...
{
	// Reset world
	if (g_physicsEngine->GetWorld() != NULL)
	{
		...
		for (int i = g_physicsEngine->GetWorld()->getNumConstraints() - 1; 
			i >= 0; --i)
		{
			btTypedConstraint* typed = g_physicsEngine->GetWorld()->getConstraint(i);
			g_physicsEngine->GetWorld()->removeConstraint(typed);
			SAFE_DELETE(typed);
		}

		// remove all rigid body
		btCollisionObjectArray& array =  g_physicsEngine->GetWorld()->getCollisionObjectArray();
		int Nums = g_physicsEngine->GetWorld()->getNumCollisionObjects();
		for (int i = Nums - 1; i >= 0; --i)
		{
			...
			g_physicsEngine->GetWorld()->removeCollisionObject(obj);
			SAFE_DELETE(obj);
		}
	}
}
...
void RagDoll::CreateHinge(Bone* parent, OBB* A, OBB* B, float upperLimit, float lowerLimit, D3DXVECTOR3& hingeAxisA, D3DXVECTOR3& hingeAxisB, bool ignoreCollisions /*= true*/)
{
	...
	g_physicsEngine->GetWorld()->addConstraint(hingeC, ignoreCollisions);
}

void RagDoll::CreateTwistCone(Bone* parent, OBB* A, OBB* B, float limit, D3DXVECTOR3& hingeAxisA, D3DXVECTOR3& hingeAxisB, bool ignoreCollisions /*= true*/)
{
	if(parent == NULL || A == NULL || B == NULL)
		return;
