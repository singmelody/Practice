...
HairPatch::HairPatch(ControlHair* pCH1, ControlHair* pCH2, ControlHair* pCH3, ControlHair* pCH4)
{
...
std::vector<D3DXVECTOR2> HairPatch::GetStripPlacements(float sizePerHairStrip)
{
	//Place hair strips at random
	std::vector<D3DXVECTOR2> strips;
	for(int i=0; i<200; i++)
	{
...
void HairPatch::CreateHairStrips(int numSegments, float sizePerHairStrip, float stripSize)
{
	...
	std::vector<D3DXVECTOR2> strips = GetStripPlacements(sizePerHairStrip);
...
void HairPatch::Render()
{
	...
	std::vector<D3DXVECTOR3> hairTable;
	for(int h=0; h<4; h++)
	{
