...
BlurFilter::BlurFilter()
  : mBlurredOutputTexSRV(0), mBlurredOutputTexUAV(0),m_blurEffect(NULL)
{
}
...
void BlurFilter::SetGaussianWeights(float sigma)
{
	...
	m_blurEffect->SetWeights(weights);
}
...
void BlurFilter::Init(ID3D11Device* device, UINT width, UINT height, DXGI_FORMAT format, ID3DX11Effect* fx)
{
	// Start fresh.
	SAFE_RELEASE(mBlurredOutputTexSRV);
	SAFE_RELEASE(mBlurredOutputTexUAV);
	...
	m_blurEffect = new BlurEffect(fx);
}
							 ...
							 int blurCount)
{
	...
	for(int i = 0; i < blurCount; ++i)
	{
		...
		m_blurEffect->HorzBlurTech->GetDesc( &techDesc );
		for(UINT p = 0; p < techDesc.Passes; ++p)
		{
			m_blurEffect->SetInputMap(inputSRV);
			m_blurEffect->SetOutputMap(mBlurredOutputTexUAV);
			m_blurEffect->HorzBlurTech->GetPassByIndex(p)->Apply(0, dc);
		...
		// VERTICAL blur pass.
		m_blurEffect->VertBlurTech->GetDesc( &techDesc );
		for(UINT p = 0; p < techDesc.Passes; ++p)
		{
			m_blurEffect->SetInputMap(mBlurredOutputTexSRV);
			m_blurEffect->SetOutputMap(inputUAV);
			m_blurEffect->VertBlurTech->GetPassByIndex(p)->Apply(0, dc);
